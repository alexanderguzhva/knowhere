...
void IndexIVFFastScan::search_preassigned(
    ...
    if (metric_type == METRIC_L2) {
        search_dispatch_implem<true>(
                n, x, k, distances, labels, assign, centroid_dis, scaler, params);
    } else {
        search_dispatch_implem<false>(
                n, x, k, distances, labels, assign, centroid_dis, scaler, params);
    }
...
void IndexIVFFastScan::search_dispatch_implem(
    ...
    if (impl == 1) {
        search_implem_1<Cfloat>(n, x, k, distances, labels, coarse_ids, coarse_dis, scaler, params);
    } else if (impl == 2) {
        search_implem_2<C>(n, x, k, distances, labels, coarse_ids, coarse_dis, scaler, params);
