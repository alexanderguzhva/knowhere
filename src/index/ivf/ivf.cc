     ...
     public:
        iterator(const IndexType* index, std::unique_ptr<float[]>&& copied_query, const BitsetView& bitset,
            ...
            if constexpr (std::is_same_v<IndexType, faiss::cppcontrib::knowhere::IndexScaNN>) {
                ...
                workspace_ = std::make_unique<IVFSQIteratorWorkspace>(
                        index_, copied_query_.get(), &ivf_search_params_);
            } else if constexpr (std::is_same_v<IndexType, IndexIVFRaBitQWrapper>) {
                workspace_ = std::make_unique<IVFRaBitQIteratorWorkspace>(
                        index_, copied_query_.get(), &ivf_search_params_);
            } else {
                workspace_ = std::make_unique<faiss::cppcontrib::knowhere::IVFBaseIteratorWorkspace>(
                        dynamic_cast<const faiss::cppcontrib::knowhere::IndexIVF*>(index_),
                        copied_query_.get(), &ivf_search_params_);
            }
        }
